...
// chain — keeps the McCabe complexity flat. The lifecycle actions
// (finish/start/archive/unarchive) share the same handler.
const ACTION_HANDLERS = {
    edit:      () => go(actionUrl('edit')),
    present:   () => go(actionUrl('present_poll'), true),
    compare:   () => go(actionUrl('compare')),
    restart:   () => go(actionUrl('restart')),
    assign:    () => go(`${pluginUrl}/workplace/collection_assign/${encodeURIComponent(pollId)}`),
    csv:       () => go(csvUrl.value),
    pdf:       () => go(pdfUrl.value, true),
    download:  () => go(`${pluginUrl}/api/download/${encodeURIComponent(pollId)}`),
    share:     () => { qrOpen.value = true },
    delete:    () => { deleteOpen.value = true },
    finish:    () => doLifecycle('finish'),
    start:     () => doLifecycle('start'),
    archive:   () => doLifecycle('archive'),
    unarchive: () => doLifecycle('unarchive'),
}
