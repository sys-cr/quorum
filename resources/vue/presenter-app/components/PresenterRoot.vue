...
// Keys → action as a lookup map instead of a long if-chain (each branch costs
// cyclomatic complexity; the map keeps the dispatcher flat). Letters are
// normalized to lowercase before the lookup so Shift/CapsLock don't matter.
// Each handler decides on any additional conditions itself.
const shortcutActions = {
    ArrowRight: () => store.next(),
    ArrowLeft:  () => store.prev(),
    ' ':        () => store.toggleCurrentVoting(),
    n:          () => store.advanceToNext(),
    l:          () => { if (isQuizPoll.value) leaderboardOpen.value = !leaderboardOpen.value },
    f:          () => toggleFullscreen(),
    q:          () => { if (canShowQr.value) qrOpen.value = !qrOpen.value },
    Escape:     () => {
...
// Should this event be ignored? Input fields (the presenter has none, but the
// check stays defensively cheap) and Space on a focused button/link — there
// Space should natively activate the element, not the global voting toggle.
const shouldIgnoreKey = (e) => {
    const t = e.target
    if (t instanceof HTMLInputElement || t instanceof HTMLTextAreaElement) return true
    return e.key === ' ' && (t instanceof HTMLButtonElement || t instanceof HTMLAnchorElement)
}
...
// Resolve the appropriate (possibly Webkit) fullscreen method and bind it to
// its object; null if the browser offers none. Keeps the toggle below flat.
const resolveFullscreenFn = () => {
