...
const onAction = (action, p) => {
    switch (action.id) {
        case 'edit':      onEdit(p); break
        case 'results':
            if (typeof window !== 'undefined') { const u = resultsUrl(p.id); if (u) window.location.href = u }
            break
        case 'present':
            ...
            break
        case 'share':     qrPoll.value = p; break
        case 'download':
            if (typeof window !== 'undefined') window.location.href = downloadUrl(p.id)
            break
        case 'compare':   navigate(withCid(`workplace/compare/${p.id}`)); break
        case 'restart':   onDuplicate(p); break
        case 'start':     runLifecycle(p, 'startVoting'); break
        case 'finish':    runLifecycle(p, 'stopVoting'); break
        case 'archive':   runLifecycle(p, 'archiveVoting'); break
        case 'unarchive': runLifecycle(p, 'unarchiveVoting'); break
        case 'remove':    onRemove(p); break
    }
}
