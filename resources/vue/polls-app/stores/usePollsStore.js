...
export const usePollsStore = defineStore('polls', {
    ...
    actions: {
        async loadPoll(token) {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    if (res.status === 410) {
                        // Distinguish waiting from ended. If a sibling question
                        // of the collection is already running, follow it
                        // directly instead of waiting.
                        const activeToken = body?.active_token ?? null
                        if (activeToken && activeToken !== token) {
                            this.loading = false
                            await this.followTo(activeToken)
                            return
                        }
                        if ((body?.status ?? 'ended') === 'paused') {
                            ...
                            this.ended = true
                            // Question has (already) ended — try the solution.
                            // If this browser already voted, submittedAnswer is
                            // set by readVote (above), so the verdict applies.
                            this.maybeLoadSolution()
                        }
                    } else {
                        this.error = body?.message ?? `HTTP ${res.status}`
                    }
        ...
        async checkStatus() {
            ...
            const isOwnActive = status?.status === 'active' && (!activeToken || activeToken === this.token)

            if (this.isAnswered) {
                // Thanks state: only react again when a DIFFERENT question
                // starts (no re-vote on the already answered question).
                if (activeToken && activeToken !== this.token) {
                    await this.followTo(activeToken)
                } else if (status?.status === 'ended') {
                    // Deliberately do NOT null currentPoll — the learning block
                    // needs quiz_mode/options. The template checks `ended`
                    // before `hasPoll`, so the end state stays correct.
                    this.ended   = true
                    this.waiting = false
                    this.maybeLoadSolution()
                }
            ...
            if (isOwnActive) {
                await this.loadPoll(this.token)
            } else if (activeToken && activeToken !== this.token) {
                await this.followTo(activeToken)
            } else if (status?.status === 'ended' && !this.ended) {
                this.ended   = true
                this.waiting = false
                this.maybeLoadSolution()
            }
