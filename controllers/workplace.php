...
class WorkplaceController extends PluginController
{
    ...
    public function create_action(): void
    {
        ...
        // checkbox (1) — if the field is missing entirely, it counts as public
        // (default 1).
        $resultsPublic = \Request::int('results_public', 1) === 1;

        // The form delivers flat strings → convert to the service format
        // `[{label: …}, …]`. Free-text has no options — pass an empty array.
        $options = [];
        if ($type !== 'freitext') {
        ...
        try {
            ...
            if ($collectionId !== '') {
                ...
                try {
                    $collections->addPollToCollection($collectionId, $poll->id, $userId);
                    ...
                    // Collection vanished/foreign in the meantime: the poll
                    // still exists — no rollback, just an honest message.
            ...
            // Carry cid/collection so the re-rendered form stays in the right
            // context (course frame or direct-in-collection creation).
            ...
            $this->redirect(\PluginEngine::getURL($this->plugin, $params, 'workplace/new', false));
        }
    }
    ...
    public function update_action(string $pollId = ''): void
    {
        ...
        // Result visibility (opt-out) — changeable at any time, including with
        // responses.
        $resultsPublic = \Request::int('results_public', 1) === 1;
