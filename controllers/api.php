...
class ApiController extends PluginController
{
     ...
     */
    private function mayStreamPoll(Poll $poll): bool
    {
        $userId  = $GLOBALS['user']->id ?? '';
        $isOwner = ($userId !== '' && $userId === $poll->userId);
        $hasSemPerm = $poll->seminarId !== null
            && $GLOBALS['perm']->have_studip_perm('tutor', $poll->seminarId);
        return $isOwner || $hasSemPerm;
    }

    /**
     * SSE loop: send aggregated counts every 2 s (only on change), heartbeat
     * every 30 s. Aborts via `connection_aborted()` or a hard 1 h limit
     * (worker protection; the client reconnects automatically).
     */
    private function runSseStream(string $pollId): void
    {
        $deadline       = time() + 3600;
        $lastHeartbeat  = time();
        $lastCounts     = null;
    ...
    public function course_student_results_action(string $pollId = ''): void
    {
        ...
        if (!$this->mayViewStudentResults($poll)) {
            $this->renderJsonError(404, 'poll_not_found', 'Diese Abstimmung existiert nicht.');
            return;
        }

        $this->renderJson(200, $this->buildStudentResultsPayload($repo, $poll, $pollId));
    }

    /**
     * Access predicate for the student results review: course member, results
     * released (`results_public`), poll finished (not effectively active) and
     * not archived. Otherwise `false` → the caller renders 404 (no
     * existence/status oracle).
     */
    private function mayViewStudentResults(?Poll $poll): bool
    {
        if ($poll === null) {
            return false;
        }
    ...
    /**
     * Assembles the results payload per question type (aggregates only, no
     * identity linkage). The access/status check has already happened at the
     * call site — the question is guaranteed to have ended.
     *
     * @return array<string,mixed>
     */
    private function buildStudentResultsPayload(PollsRepository $repo, Poll $poll, string $pollId): array
    {
        $payload = [
            'type'     => $poll->type,
    ...
    public function export_action(string $pollId = ''): void
    {
        ...
        $content = $this->buildExportContent($repo, $poll, $format);

        if ($format === 'pdf') {
            $this->dispatchExportPdf($content, $poll->token);
            return;
        }
        $this->dispatchExportCsv($content, $poll->token);
    }

    /**
     * Builds the export content per question type and format. Export applies to
     * ALL types (user feedback): choice as an option/votes/percent table,
     * free text as a response list, matrix as a rows×scale table. Pure
     * data→string assembly; the HTML/CSV generation is done by the unit-tested
     * exporters.
     */
    private function buildExportContent(PollsRepository $repo, Poll $poll, string $format): string
    {
    ...
    /**
     * PDF: Stud.IP's `\ExportPDF` (TCPDF) with the standard header/footer — the
     * HTML comes from the unit-tested exporters, this is rendering only.
     * `dispatch()` streams the PDF inline to the browser.
     */
    private function dispatchExportPdf(string $html, string $token): void
    {
        $doc = new \ExportPDF();
        $doc->setHeaderTitle('Quorum');
        $doc->addPage();
        $doc->writeHTML($html);
        $doc->dispatch('quorum-poll-' . $token);
        $this->render_nothing();
    }

    private function dispatchExportCsv(string $content, string $token): void
    {
        $this->set_status(200);
        $this->set_content_type('text/csv; charset=utf-8');
        // Token is the public slug (no PII) and filename-safe.
        header('Content-Disposition: attachment; filename="quorum-poll-' . $token . '.csv"');
    ...
    {
        $userId = $this->requirePostUserWithCsrf();
        if ($userId === null) return;
        ...
        $this->createPollFromBody($userId, $body, $options, $seminarId);
    }

    /**
     * Shared entry guard for state-changing JSON POST endpoints: POST method,
     * logged-in owner, valid CSRF token. Returns the user id or renders the
     * matching error response (405/401/403) and returns `null`.
     */
    private function requirePostUserWithCsrf(): ?string
    {
        if (\Request::method() !== 'POST') {
            $this->renderJsonError(405, 'method_not_allowed', 'POST erforderlich.');
            return null;
    ...
    /**
     * Calls `createPoll` with the (already seminar-validated) body values and
     * renders the 201 success or 422 validation response. Options are checked
     * as an array by the caller.
     *
     * @param array<string,mixed> $body
     * @param array<mixed>        $options
     */
    private function createPollFromBody(string $userId, array $body, array $options, ?string $seminarId): void
    {
        try {
            $poll = $this->service->createPoll(
                ...
                options:         array_is_list($options) ? array_values($options) : $options,
                seminarId:       $seminarId,
