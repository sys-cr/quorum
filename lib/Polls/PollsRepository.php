...
class PollsRepository
{
    ...
    private static function rowToPoll(array $row): Poll
    {
        ...
        $seminarId     = $row['seminar_id']          ?? null;
        $archivedAt    = $row['archived_at']         ?? null;
        $parentId      = $row['parent_poll_id']      ?? null;
        $collectionId  = $row['collection_id']       ?? null;
        $collectionPos = $row['collection_position'] ?? null;
        $expiresAt     = $row['expires_at']          ?? null;
        return new Poll(
            id:                 (string) ($row['id'] ?? ''),
            token:              (string) ($row['token'] ?? ''),
            seminarId:          ($seminarId === null || $seminarId === '') ? null : (string) $seminarId,
            userId:             (string) ($row['user_id'] ?? ''),
            question:           (string) ($row['question'] ?? ''),
            type:               (string) ($row['type'] ?? 'mc'),
            options:            is_array($options) ? $options : [],
            isActive:           (bool)   ($row['is_active'] ?? false),
            mkdate:             (int)    ($row['mkdate'] ?? 0),
            chdate:             (int)    ($row['chdate'] ?? 0),
            archivedAt:         $archivedAt === null ? null : (int) $archivedAt,
            parentPollId:       ($parentId === null || $parentId === '') ? null : (string) $parentId,
            collectionId:       ($collectionId === null || $collectionId === '') ? null : (string) $collectionId,
            collectionPosition: $collectionPos === null ? null : (int) $collectionPos,
