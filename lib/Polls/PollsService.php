...
final class PollsService
{
    ...
    public function statusForToken(string $token): array
    {
        ...
        $activeToken = $this->autoFollowToken($poll, $now);
    ...
    /**
     * Auto-follow target for a paused/ended question: returns the token of the
     * currently active sibling question (collection) or follow-up round
     * (compare chain) — or null. Collection takes precedence; the chain search
     * only kicks in when no collection was followed.
     */
    private function autoFollowToken(Poll $poll, int $now): ?string
    {
        $token = $this->collectionFollowToken($poll, $now);
        if ($token !== null) {
            return $token;
        }
        ...
        return $this->chainFollowToken($poll, $now);
    }

    /** Active sibling token of the same collection — or null. */
    private function collectionFollowToken(Poll $poll, int $now): ?string
    {
        ...
        return ($sibling !== null && $sibling->isEffectivelyActive($now)) ? $sibling->token : null;
    }

    /** Active token of the running follow-up round of the same compare chain — or null. */
    private function chainFollowToken(Poll $poll, int $now): ?string
    {
    ...
    public function recordResponse(string $pollId, array $payload): Response
    {
        ...
        // Quiz mode: nickname (opt-in) + server-side score. The `nickname` key
        // is removed from the payload so it does not also end up raw in the
        // response.
        [$nickname, $score] = self::quizFieldsFor($poll, $payload, $now);
    ...
    /**
     * Quiz mode: determines nickname + points for a response. Outside quiz mode
     * `[null, null]`.
     *
     * Points are computed exclusively server-side (no client trust). The
     * nickname is an active opt-in (empty = the answer counts but does not
     * appear on the leaderboard) and length-limited — no real name, not
     * mandatory.
     *
     * @param  array<string,mixed> $payload
     * @return array{0: ?string, 1: ?int}
     */
    private static function quizFieldsFor(Poll $poll, array $payload, int $now): array
    {
        if (!$poll->quizMode) {
            return [null, null];
        }
        $raw      = trim((string) ($payload['nickname'] ?? ''));
        $nickname = $raw !== '' ? mb_substr($raw, 0, 40) : null;
        ...
        // remaining time = expires_at − now. Without a timer only correctness
        // counts (full points).
    ...
    private static function validateMatrixOptions(array $options): void
    {
        ...
        // Order of checks deliberately preserved (first both min-2, then both
        // max) so with multiple violations the same message as before applies
        // first.
        self::assertMatrixMin($rows, 'Matrix benötigt mindestens zwei Zeilen (rows).');
        self::assertMatrixMin($scale, 'Matrix benötigt mindestens zwei Skalenwerte (scale).');
        self::assertMatrixMax($rows, self::MAX_MATRIX_ROWS, 'Maximal %d Matrix-Zeilen erlaubt.');
        self::assertMatrixMax($scale, self::MAX_MATRIX_SCALE, 'Maximal %d Skalenwerte erlaubt.');
    }

    /** Min-2 check for a filtered matrix list (rows/scale). */
    private static function assertMatrixMin(array $list, string $message): void
    {
        if (count($list) < 2) {
    ...
    /** Upper-bound check; `$messageFmt` carries exactly one `%d` for `$max`. */
    private static function assertMatrixMax(array $list, int $max, string $messageFmt): void
    {
        if (count($list) > $max) {
            throw new InvalidResponseException(sprintf($messageFmt, $max));
        }
    }
